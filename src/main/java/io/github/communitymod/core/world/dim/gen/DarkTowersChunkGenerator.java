    ...
    public void buildSurfaceAndBedrock(WorldGenRegion region, ChunkAccess chunkAccess) {
        ...
        for (var x = 0; x < 16; ++x) {
            for (var z = 0; z < 16; ++z) {
                ...
                final var height = new Random(System.currentTimeMillis()).nextInt(maxHeightDifference) + baseHeight;
                final var block = ForgeRegistries.BLOCKS.getValue(new ResourceLocation(this.settings.towerBlock));
